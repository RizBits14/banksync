...
export const scanUpload = async (
    ...
    try {
        ...
        const result = await scanUploadDataQuality(uploadId);
...
export const getDataQualityIssues = async (
    ...
    try {
        ...
        if (req.query.issueType) {
            filter.issueType = String(req.query.issueType);
        }
        ...
        const issues = await DataQualityIssue.find(filter)
            .populate(
                "uploadId",
                "originalName sourceSystem status"
            )
            .populate("primaryTransactionId")
        ...
        return res.status(500).json({
            success: false,
            message: "Unable to retrieve data quality issues",
        });
    }
};
...
export const getDataQualitySummary = async (
    ...
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Unable to retrieve data quality summary",
        });
    }
};